...
- (void)beginLoad {
	...
	else if ([[NSFileManager defaultManager] fileExistsAtPath:[scriptView pathForLibraryResource:path]]) {
		NSLog(@"Loading Image from library (lazy): %@", path);
		fullPath = [scriptView pathForLibraryResource:path];
  }
	else {
